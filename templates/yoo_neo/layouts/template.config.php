...
if ($config->get('load_googlefonts', false)) {
	...
	if(in_array($color, array('retrobeige','retrolandscape','retropaper'))){
		$this->warp->stylesheets->add('http://fonts.googleapis.com/css?family=Yanone+Kaffeesatz:regular,light|Lobster');
	}
	
	if(in_array($color, array('gradientblue','gradientbusiness','gradientred','gradientribbon','gradientspace'))){
		$this->warp->stylesheets->add('http://fonts.googleapis.com/css?family=Nobile:regular,bold');
	}
}
...
if ($this->warp->useragent->browser() == 'msie' && $this->warp->useragent->version() == '6.0') {
	...
	/* if ($color != '' && $color != 'default') {
	    $ie6[] = sprintf($css, $this->warp->path->url("css:variations/{$color}-ie6hacks.css"));
	} */
	$ie6[] = sprintf($js, $this->warp->path->url('js:addons/ie6fix.js'));
	$ie6[] = sprintf($js, $this->warp->path->url('js:addons/ie6png.js'));
...
// add class="nofrontpage" to body tag if frontpage, category blog or section blog
