...
Element.extend({
	...
	// with mt1.2.4 compat layer selectbox.getValue() doesnt seem to work in IE8 so implement at least selectbox.get('value')
	// for mt1.1
	get:function(v) {
		// $$$ hugh - added getText, at some point might want to do something like ...
		//
		// var func = 'get' + upperCase(v);
		// try(eval(func()){
		// ...
		// }
		//
		// ... as get('foo') seems to map to getFoo() reliably.  But for now, just check for 'value' and 'text'
		if (v == 'value') {
			...
			fconsole('using mootools 1 and element.get() call to unimplemented parameter: ' + v);
		}
	},
