var fabrikCalendar = new Class({
	initialize: function(el) {
		...
		this.fx = {};
		this.ajax = {};
		this.fx.showMsg = new Fx.Styles(this.el.getElement('.calendar-message'), {'duration':700});
	...
	receiveMessage: function(senderBlock, task, taskStatus, json) {
		if(this.listenTo.indexOf(senderBlock) != -1) {
			if(task == 'updateRows') {
				...
				oPackage.closeMocha('addeventwin');
				this.showMessage('Event added');
			}
			if(task == 'addEvForm') {
				this.addEvForm(json);
				oPackage.closeMocha('chooseeventwin');
			}
		}		
		this.showView();
	},
	...
	_makeEventRelDiv: function(entry, opts, aDate)
	{
		...
		var style = {
			...
			var id = 'fabrikEvent_' + entry._tableid + '_' + entry.id; 
	...
	doPopupEvent: function(e, entry, label){
		...
		if(this.options.mooversion > 1.1) {
			...
			var newtop = loc.top - this.popWin.getSize().size.y;
		}
		var fxopts = {
		    'opacity':[0, 1],
	...
	showMonth: function(){
				...
				this.entries.each(function(entry) {
					...
					|| (entry.enddate == '' && entry.startdate.isSameDay(firstDate))) {
						...
						if(this.options.mooversion > 1.1) {
							...
							top = (td.getSize().size.y * (i-1)) +  this.el.getElement('.monthView .dayHeading').getSize().size.y+ td.getElement('.date').getSize().size.y;
							this.colwidth['.monthView'] = this.colwidth['.monthView'] ? this.colwidth['.monthView'] : td.getSize().size.x;
						}
		...
		this.entries.each(function(entry) {
			var item = this.el.getElement('.fabrikEvent_' + entry._tableid + '_' + entry.id);
			if(item) {
	...
	_makePopUpWin: function() {
		if($type(this.popup) == false) {
			...
			this.fx.showEventActions = new Fx.Styles(this.popup, {
	...
	_buildEventOpts:function(opts)
	{
		...
		if(this.options.mooversion > 1.1) {
			...
			var rowheight = td.getSize().size.y;
			this.colwidth[opts.divclass] = this.colwidth[opts.divclass] ? this.colwidth[opts.divclass] : td.getSize().size.x;
			top = this.el.getElement(opts.divclass).getElement('tr').getSize().size.y;
		}
		...
		if (this.options.mooversion > 1.1) {
	  	left += this.el.getElement(opts.divclass).getElement('td').getSize().x;
			...
			left += this.el.getElement(opts.divclass).getElement('td').getSize().size.x;
		}
	...
	addEvForm: function(o)
	{
		...
		// stop the doubleclickdate being used when editing a record added in the current session #816
		delete this.doubleclickdate;
	...
	showView: function (view) {
		...
		Cookie.set("fabrik.viz.calendar.view", this.options.viewType);
	},
	...
	render: function(stuff, options) {
		...
		// $$$ hugh - didn't seem to work with Object.extend, changed to $extend
		...
		if($type(this.el.getElement('.addEventButton')) !== false) {
			this.el.getElement('.addEventButton').addEvent('click', this.openAddEvent.bindAsEventListener(this));
		}
		var bs = [];
		bs.push(new Element('li', {'class':'centerOnToday'}).appendText(this.aText.today));
		if(this.options.show_day != 0) {
			bs.push(new Element('li', {'class':'dayViewLink'}).appendText(this.aText.day));
		}
		if(this.options.show_week != 0) {
			bs.push(new Element('li', {'class':'weekViewLink'}).appendText(this.aText.week));
		}
		bs.push(new Element('li', {'class':'monthViewLink'}).appendText(this.aText.month));
		...
		if ($type(Cookie.get('fabrik.viz.calendar.date')) !== false) {
			this.date = new Date(Cookie.get('fabrik.viz.calendar.date'));
		}
		var startview = $type(Cookie.get("fabrik.viz.calendar.view")) === false ? this.options.viewType : Cookie.get("fabrik.viz.calendar.view");
		switch(startview){
			case 'dayView':
				this.renderDayView();
				break;
		...
		this.el.getElement('.nextPage').addEvent('click',  this.nextPage.bindAsEventListener(this));
		this.el.getElement('.previousPage').addEvent('click',  this.previousPage.bindAsEventListener(this));
		if(this.options.show_day != 0) {
			this.el.getElement('.dayViewLink').addEvent('click', this.renderDayView.bindAsEventListener(this));
		}
		if(this.options.show_week != 0) {
			this.el.getElement('.weekViewLink').addEvent('click', this.renderWeekView.bindAsEventListener(this));
		}
		this.el.getElement('.monthViewLink').addEvent('click', this.renderMonthView.bindAsEventListener(this));
		this.el.getElement('.centerOnToday').addEvent('click', this.centerOnToday.bindAsEventListener(this));
		this.showMonth();
		this.watchFilters();
		//clear filter list
	...
	showMessage: function(m) {
		this.el.getElement('.calendar-message').setHTML(m);
	...
	nextPage: function() {
		...
		Cookie.set('fabrik.viz.calendar.date', this.date);
	},
