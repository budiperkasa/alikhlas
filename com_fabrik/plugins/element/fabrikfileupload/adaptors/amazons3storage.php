...
class amazons3storage extends storageAdaptor{
	...
	{
		if (substr($filepath, 0, strlen( COM_FABRIK_BASE.DS)) == COM_FABRIK_BASE.DS) {
			$filepath = Fabrikstring::ltrimword($filepath, COM_FABRIK_BASE.DS);
		}
		return $filepath;
	}
	...
	function write($file, $buffer)
	{
		...
		$bucket = $this->getBucketName();
		$acl = $this->getAcl();
		if ($this->s3->putObject($buffer, $bucket, $file, $acl)) {
			return true;
		}else{
			return false;
		}
	...
	function _getThumb($file)
	{
		...
		$ulDir = COM_FABRIK_BASE . DS. $params->get('ul_directory');
		$ulDir = $this->clean($ulDir);
		$ulDir = $w->parseMessageForPlaceHolder($ulDir);

		$thumbdir = $this->clean(COM_FABRIK_BASE.DS.$params->get('thumb_dir'));
	...
	function _getCropped( $file )
	{
		...
		$thumbdir = $this->clean(COM_FABRIK_BASE.DS.$params->get('fileupload_crop_dir'));
		$thumbdir = $w->parseMessageForPlaceHolder($thumbdir );
