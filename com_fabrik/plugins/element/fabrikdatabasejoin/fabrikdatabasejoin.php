...
class FabrikModelFabrikDatabasejoin  extends FabrikModelElement {
		...
		if ($this->canUse()) {
			...
			switch ($displayType) {
				...
				case 'checkbox':
					...
					break;
				case 'auto-complete':
					$str .= "<input type=\"text\" size=\"20\" name=\"{$thisElName}-auto-complete\" id=\"{$id}-auto-complete\" value=\"$defaultLabel\" class=\"fabrikinput inputbox autocomplete-trigger\"/>";
	...
	function getValue($data, $repeatCounter = 0, $opts = array())
	{
		...
		if (!array_key_exists($valueKey, $this->defaults)) {
			...
			$value = JArrayHelper::getValue($opts, 'use_default', true) == false ? array() : $this->getDefaultValue($data);

			// $$$rob -huh? why are we redoing this here then???
			// $$$ hugh - buggered if I know.  I don't remember putting this in.  Suggest we take it
			// out and see if everything goes to hell in a handbasket.
			...
			if (!array_key_exists('valueFormat', $opts)) { $opts['valueFormat'] = 'raw'; }
			...
			if ($groupModel->isJoin()) {
				...
				// $$$ rob 10/06/2011 - moved from below
				...
				if ($groupModel->canRepeat()) {
					...
					if (array_key_exists($name, $data)) {
						if (is_array($data[$name])) {
							//occurs on form submission for fields at least AND validatons
							$a = $data[$name];
						} else {
			...
			// $$$ rob - commenting out as value can be '' from ajax validation in repeat group (2nd repeat group)
			// and this line will replace '' with the firs repeat groups value. As the comment says its for joined data - moving it up
			// to where we deal wth joined data.
			/*if ($value === '') { //query string for joined data
				$value = JArrayHelper::getValue($data, $name, $value);
			}*/
			$formModel =& $this->getForm();
			...
			FabrikHelperHTML::debug($value, 'dbjoin value for:' . $element->label);
		}
