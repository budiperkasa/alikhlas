...
function fabrikParseRoute($segments)
{
	...
	switch ($segments[0]) { //view (controller not passed into segments)
		...
		case 'details':
			$vars['task'] 		= 'view';
			$vars['fabrik'] 	= JArrayHelper::getValue($segments, 1, 0);
			// $$$ rob no longer passing in tableid - no need and makes sef urls tidier
			//$vars['tableid'] 	= JArrayHelper::getValue($segments, 2, 0);
		...
		case 'table':
			...
			$vars['tableid'] 	= JArrayHelper::getValue($segments, 1, 0);
