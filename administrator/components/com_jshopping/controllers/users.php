...
class JshoppingControllerUsers extends JController{
    ...
    function save() {
        ...
        $user_site =  new JUser($user_id);  
        $user_site->email =  $post['email']; 
        $user_site->block =  $post['block'];  
        $user_site->username =  $post['u_name'];  
        $user_site->name =  $post['f_name'];  
        if ($post['password']){
             jimport('joomla.user.helper'); 
             $salt  = JUserHelper::genRandomPassword(32);
             $crypt = JUserHelper::getCryptedPassword($post['password'], $salt);             
             $user_site->password = $crypt.':'.$salt;            
        }
        ...
        if (!$user_site->save(true)) {
            JError::raiseWarning("", $user_site->getError());
            $this->setRedirect("index.php?option=com_jshopping&controller=users&task=edit&user_id=".$user_shop->user_id);
            return 0;
        }
        ...
        $dispatcher->trigger( 'onAfterSaveUser', array(&$user_site, &$user_shop) );
