...
class plgSearchZoosearch extends JPlugin {
	...
	*/
	function onSearch($text, $phrase = '', $ordering = '', $areas = null) {
		...
		if (!empty($items)) {
			...
			foreach ($items as $item) {
				...
				if ($menu_item = self::_findItem($item)) {
					$row->href .= '&Itemid=' . $menu_item->id;
				}
	...
	protected function 	_findItem(Item $item)
	{
		$menu_items	= JApplication::getMenu('site', array())->getItems('componentid', JComponentHelper::getComponent('com_zoo')->id);

		foreach($menu_items as $menu_item)
		{
			$menu_params = new JParameter($menu_item->params);

			if ((@$menu_item->query['view'] == 'item') && ($menu_params->get('item_id') == $item->id)) {
		...
		foreach($menu_items as $menu_item)
		{
			...
			if ((@$menu_item->query['view'] == 'category') && in_array($menu_params->get('category'), $item->getRelatedCategoryIds())) {
					return $menu_item;
			}
		}		
